...
# ── Regime-switching factor weight sets ───────────────────────────────────────
# Applied as a second layer on top of IC-selection (|t-stat|>2).
# Only factors that BOTH passed IC selection AND have a non-zero regime weight
# are used in composite scoring.

BULL_WEIGHTS: dict[str, int] = {
    'Momentum_252D':      1,   # trend following works in bull
    'Volatility_20D':     1,   # high vol → high return in bull (risk-on)
    'Volume_Zscore_20D': -1,   # high volume = crowded trade reversal
    'Amihud_20D':        -1,   # large-cap growth: illiquidity reversal
    'PB_Ratio':           1,   # growth premium dominates
    'PIN':               -1,   # informed trading negatively correlated with return
    'Filing_Sentiment':   1,   # positive tone → positive return
}

NEUTRAL_WEIGHTS: dict[str, int] = {
    'Momentum_252D':      1,   # still positive but softer
    'Volatility_20D':     0,   # vol signal unclear in transitional regime
    'Volume_Zscore_20D': -1,
    'Amihud_20D':        -1,
    'PB_Ratio':           1,
    'ROE':                1,   # quality starts to matter
    'PIN':               -1,
    'Filing_Sentiment':   1,
}

BEAR_WEIGHTS: dict[str, int] = {
    'Momentum_252D':      0,   # momentum crashes in bear markets
    'Volatility_20D':    -1,   # flip: low vol = safe haven (defensive)
    'Volume_Zscore_20D': -1,
    'Amihud_20D':         0,   # illiquidity signal unreliable in stress
    'PB_Ratio':          -1,   # flip: value over growth in bear
    'ROE':                1,   # quality/profitability as defensive factor
    'ROA':                1,   # asset efficiency as defensive factor
    'PIN':               -1,
    'Filing_Sentiment':   1,
}


def get_regime_weights(bear_score: int) -> dict[str, int]:
    """
    Return the factor weight dict for the given bear_score (0–3).

    Bear score mapping:
      0  → BULL    (no bearish indicators)
      1  → NEUTRAL (one bearish indicator — caution)
      2+ → BEAR    (two or three bearish indicators — defensive)
    """
    if bear_score == 0:
        return BULL_WEIGHTS
    elif bear_score == 1:
...
def _compute_bear_count(
    ...
    """
    Internal helper: download VIX + SPX, compute 3 bear indicators, return
    daily integer bear_count (0-3) trimmed to [start, end].
    """
    load_start = (
    ...
    regime_df = pd.concat([ind_vix, ind_ma, ind_mom], axis=1).dropna()
    regime_df.columns = ['vix', 'ma', 'momentum']
    return regime_df.sum(axis=1).astype(int).loc[start:]


def compute_bear_score_series(
    start: str,
    end: str,
    vix_threshold: float = 25.0,
    ma_window: int = 200,
    ...
    """
    Return daily integer bear_score (0–3) from start to end.

    Same three indicators as compute_regime_multiplier:
      VIX > vix_threshold, SPX < 200-day MA, SPX 12M momentum < 0.
    Result is forward-filled to business-day frequency.
    """
    bear_count = _compute_bear_count(
        start, end, vix_threshold, ma_window, momentum_window, warmup_days
    )
    bear_count = bear_count.reindex(
        pd.date_range(start, end, freq='B'), method='ffill'
    ).dropna().astype(int)
    return bear_count.rename('bear_score')
...
def compute_regime_detail(
    ...
    Returns DataFrame with columns:
      vix, ma, momentum, bear_count, multiplier, spx_close, vix_level
    """
    load_start = (
        pd.Timestamp(start) - pd.Timedelta(days=warmup_days)
    ).strftime('%Y-%m-%d')
