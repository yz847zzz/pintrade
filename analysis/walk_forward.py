...
from pintrade.backtest.regime import (
    compute_regime_multiplier, compute_regime_detail,
    compute_bear_score_series, get_regime_weights,
)
...
# ── Regime-switching composite score ─────────────────────────────────────────

def _regime_composite_score(
    factors_df: pd.DataFrame,
    bear_score_series: pd.Series,
    is_weights: dict,
) -> pd.Series:
    """
    Compute composite scores using regime-switched factor weights.

    Process per trading date:
      1. Look up bear_score for that date.
      2. Get the regime weight set (BULL / NEUTRAL / BEAR).
      3. Intersect with IS-selected factors (is_weights keys where weight != 0).
      4. Compute weighted sum for all tickers on that date.

    Falls back to equal-weight over IS-selected factors if the intersection
    yields no active factors (e.g. regime weight = 0 for all selected factors).
    """
    from collections import defaultdict

    selected = {f for f, w in is_weights.items() if w != 0}
    dates = factors_df.index.get_level_values('Date').unique()

    # Group dates by their integer bear_score (0-3)
    bs_to_dates: dict[int, list] = defaultdict(list)
    for d in dates:
        bs = int(bear_score_series.get(d, 0))
        bs_to_dates[bs].append(d)

    pieces = []
    for bs, group_dates in bs_to_dates.items():
        rw = get_regime_weights(bs)
        # Intersect: only use factors that passed IS IC AND have non-zero regime weight
        effective_w = {f: rw[f] for f in selected if rw.get(f, 0) != 0}
        if not effective_w:
            # Fallback: use IS-selected factors with their IC-derived sign weights
            effective_w = {f: is_weights[f] for f in selected}

        mask = factors_df.index.get_level_values('Date').isin(group_dates)
        pieces.append(get_composite_score(factors_df[mask], weights=effective_w))
...
def run_walk_forward(
    ...
    use_regime_weights    : if True, dynamically switch factor weights per
                            rebalance day based on bear_score (0-3).
                            Requires use_regime=True.
                            BULL (score=0) / NEUTRAL (score=1) / BEAR (score≥2)
                            weight sets are intersected with IS-selected factors.
    ...
    full_bear_score: pd.Series | None = None
    if use_regime:
        ...
        if use_regime_weights:
            full_bear_score = compute_bear_score_series(
                start=str(load_start.date()),
                end=end,
                vix_threshold=vix_threshold,
            )
            dist = full_bear_score.value_counts().sort_index()
            label_map = {0: 'bull', 1: 'neutral', 2: 'half-bear', 3: 'full-bear'}
            dist_str = '  '.join(
                f"{label_map.get(k, k)}={v/len(full_bear_score)*100:.0f}%"
                for k, v in dist.items()
            )
            print(f"  Bear score dist: {dist_str}")
    ...
    for i, (train_start, train_end, test_start, test_end) in enumerate(folds):
        ...
        # ── Slice bear score to IS / OOS windows ─────────────────────────────
        is_bear_score  = None
        oos_bear_score = None
        if full_bear_score is not None:
            is_bear_score  = full_bear_score.reindex(is_dates,  method='ffill').dropna()
            oos_bear_score = full_bear_score.reindex(oos_dates, method='ffill').dropna()

        # ── IS backtest (L/S, for IS Sharpe reference) ───────────────────────
        if use_regime_weights and is_bear_score is not None:
            is_signals = _regime_composite_score(is_factors, is_bear_score, weights)
        else:
        ...
        # ── OOS backtest ──────────────────────────────────────────────────────
        if use_regime_weights and oos_bear_score is not None:
            oos_signals = _regime_composite_score(oos_factors, oos_bear_score, weights)
        else:
